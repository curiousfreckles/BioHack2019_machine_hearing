...
double UbiSOM::evklDist(int mapWeights, const double* data)
{
	double sum = 0;
	for (int i = 0; i < variableNumber; i++)
		sum += pow(map[mapWeights][i] - data[i], 2);
	return sqrt(sum);
}
...
{
	int index = 0;
	double minDist = evklDist(0, data);
	for (int i = 1; i < neuronNumber; i++)
	{
		double dist = evklDist(i, data);
		if (dist < minDist)
		{
			minDist = dist;
			index = i;
...
void UbiSOM::train(const string& path,bool isNorm)
{
	string newPath=path;
	if (!isNorm)
	{
		newPath = "norm" + path;
		normalizeDatabyMinMax(path);
	}
	ifstream file;
...
void UbiSOM::normalizeDatabyMinMax(const string& path)
{
	string newPath = "norm"+path;
	...
	cout << "Vectors normilized" << endl; 
	f.close();
	normFile.close();
	delete[]data;
}

istream& UbiSOM::train(istream& input)
{
	double* data = new double[variableNumber];
	int j = 0;
	while (input.good())
...
int UbiSOM::learnVector(const double* data)
{
...
void UbiSOM::normalizeVectorbyMinMax(double* data)
{
	for (int i = 0; i < variableNumber; i++)
	{
